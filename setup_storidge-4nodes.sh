...
export VAGRANT_STORIDGE_CLUSTER_NODES=4
...
vagrant upload storidge-4/id_rsa.pub /tmp/id_rsa.pub.4 storidge-1
...
vagrant ssh storidge-1 --no-tty -c "cat /tmp/id_rsa.pub.4 | sudo tee -a /root/.ssh/authorized_keys"
...
vagrant upload storidge-4/id_rsa.pub /tmp/id_rsa.pub.4 storidge-2
...
vagrant ssh storidge-2 --no-tty -c "cat /tmp/id_rsa.pub.4 | sudo tee -a /root/.ssh/authorized_keys"
...
vagrant upload storidge-4/id_rsa.pub /tmp/id_rsa.pub.4 storidge-3
...
vagrant ssh storidge-3 --no-tty -c "cat /tmp/id_rsa.pub.4 | sudo tee -a /root/.ssh/authorized_keys"

vagrant upload storidge-1/id_rsa.pub /tmp/id_rsa.pub.1 storidge-4
vagrant upload storidge-2/id_rsa.pub /tmp/id_rsa.pub.2 storidge-4
vagrant upload storidge-3/id_rsa.pub /tmp/id_rsa.pub.3 storidge-4
vagrant ssh storidge-4 --no-tty -c "cat /tmp/id_rsa.pub.1 | sudo tee -a /root/.ssh/authorized_keys"
vagrant ssh storidge-4 --no-tty -c "cat /tmp/id_rsa.pub.2 | sudo tee -a /root/.ssh/authorized_keys"
vagrant ssh storidge-4 --no-tty -c "cat /tmp/id_rsa.pub.3 | sudo tee -a /root/.ssh/authorized_keys"
...
vagrant ssh storidge-4 --no-tty -c "sudo ${JOIN_COMMAND} --ip 10.0.9.13"
